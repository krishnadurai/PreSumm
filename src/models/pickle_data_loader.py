...
def load_pickled_transcripts(source_file):
    transcript_utterances = pd.read_pickle(source_file)
    filtered_utterances = transcript_utterances[transcript_utterances["prediction"] == True]
    grouped = filtered_utterances.groupby(['file'])
    raw_src = dict()
    for name, group in grouped:
        print(name)
        raw_text_sentences = []
        file_name = group['file']
        for utterance in group['text']:
            sentences = utterance.split('. ')
            speaker_prefix = sentences[0].split(':')[0]
            raw_text_sentences.extend(sentences)
        raw_group_text = ' [CLS] [SEP] '.join(raw_text_sentences)
        raw_src[name] = raw_group_text
    print(raw_src)
    return raw_src

def load_text(source_text, device, save_file):
    max_pos = 512
    ...
    for file_name, x in source_text.items():
        src, mask_src, segments_ids, clss, mask_cls = _process_src(x)
        segs = torch.tensor(segments_ids)[None, :].to(device)
        src_text = [sent.replace('[SEP]','').strip() for sent in x.split('[CLS]')]
...
if __name__ == "__main__":
   source = load_pickled_transcripts('filtered_utterances.pkl')
   load_text(source, 0, 'utterances.test.0.bert.pt')
